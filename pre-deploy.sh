...
if [[ $HOST = "x86_64-apple-darwin11" ]]; then
    find $TRAVIS_BUILD_DIR -type f | grep -i pivx-qt.dmg$ | xargs -i cp {} $OUTDIR/bin
    find $TRAVIS_BUILD_DIR -type f | grep -i pivx-core.dmg$ | xargs -i cp {} $OUTDIR/bin
else
    cp $TRAVIS_BUILD_DIR/src/qt/pivx-qt $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/qt/pivx-qt.exe $OUTDIR/bin/ || echo "no QT Wallet"
    cp $TRAVIS_BUILD_DIR/src/pivxd $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/pivx.exe $OUTDIR/bin/
